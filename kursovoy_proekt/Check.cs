    ...
    public partial class Check : Form
    {
        ...
        private void SetupDataGridViewColumns()
        {
            ...
            dataGridViewServices.Columns.AddRange(
                ...
                new DataGridViewTextBoxColumn { Name = "UnitPrice", HeaderText = "Цена", Width = 90, ReadOnly = true },
                new DataGridViewTextBoxColumn { Name = "TotalPrice", HeaderText = "Сумма", Width = 100, ReadOnly = true }
            );
            dataGridViewServices.CellClick += DataGridViewServices_CellClick;
        }
        ...
        private void LoadClients(MySqlConnection conn)
        {
            ...
            using (var reader = cmd.ExecuteReader())
            {
                ...
                while (reader.Read()) comboBoxClients.Items.Add(new ClientData { Id = reader.GetInt32("id"), FIO = reader.GetString("FIO"), Passport = reader.GetString("passport_series_number") });
        ...
        {
            string query = @"SELECT h.id, h.name, hc.class, h.capacity, CASE hc.class WHEN 'Эконом' THEN 2000 WHEN 'Комфорт' THEN 3500 WHEN 'Люкс' THEN 6000 WHEN 'Премиум' THEN 9000 WHEN 'Бизнес' THEN 7000 ELSE 3000 END AS price FROM house h JOIN home_class hc ON h.home_class_id = hc.id ORDER BY h.name";
            using (var cmd = new MySqlCommand(query, conn))
            using (var reader = cmd.ExecuteReader())
            {
                comboBoxHouses.Items.Clear();
                while (reader.Read()) comboBoxHouses.Items.Add(new HouseData { Id = reader.GetInt32("id"), Name = reader.GetString("name"), Class = reader.GetString("class"), Capacity = reader.GetInt32("capacity"), PricePerDay = reader.GetDecimal("price") });
        ...
        private void LoadServices(MySqlConnection conn)
        {
            ...
            using (var reader = cmd.ExecuteReader())
            {
                ...
                while (reader.Read()) listBoxServices.Items.Add(new ServiceItem { Id = reader.GetInt32("id"), Name = reader.GetString("name_services"), Price = reader.GetDecimal("price"), Description = reader["description"]?.ToString() ?? "", Duration = reader.GetInt32("duration") });
            }
        }

        private void LoadActiveBookings(MySqlConnection conn)
        {
            string query = @"SELECT b.id, c.FIO, h.name, b.check_in_date, b.check_out_date FROM booking b JOIN client c ON b.client_id = c.id JOIN house h ON b.house_id = h.id WHERE b.status = 'confirmed' AND b.check_in_date >= CURDATE() ORDER BY b.check_in_date";
            using (var cmd = new MySqlCommand(query, conn))
            {
            ...
            foreach (DataRow row in activeBookingsTable.Rows)
                comboBoxBookings.Items.Add(new BookingItem { Id = Convert.ToInt32(row["id"]), ClientName = row["FIO"].ToString(), HouseName = row["name"].ToString(), CheckInDate = Convert.ToDateTime(row["check_in_date"]), DisplayText = $"№{row["id"]} - {row["FIO"]} - {row["name"]} - {Convert.ToDateTime(row["check_in_date"]):dd.MM.yyyy}" });
        }
        ...
        private void LoadDiscounts(MySqlConnection conn)
        {
            ...
            using (var reader = cmd.ExecuteReader())
            {
                ...
                comboBoxDiscount.Items.Add(new DiscountData { Id = 0, Name = "Без скидки", Percent = 0, MinDays = 0 });
                while (reader.Read()) comboBoxDiscount.Items.Add(new DiscountData { Id = reader.GetInt32("id"), Name = reader.GetString("name"), Percent = reader.GetDecimal("percent"), Type = reader.GetString("type"), MinDays = reader.GetInt32("min_days"), Description = reader["description"]?.ToString() ?? "" });
        ...
        {
            if (comboBoxDiscount.SelectedItem is DiscountData d)
            {
                if (d.Id == 0) { discountAmount = 0; labelDiscountInfo.Text = ""; }
                else if (stayDays >= d.MinDays) { discountAmount = (houseDailyPrice * stayDays) * d.Percent / 100; labelDiscountInfo.Text = $"✅ {d.Description}"; labelDiscountInfo.ForeColor = Color.FromArgb(46, 139, 87); }
                else { discountAmount = 0; labelDiscountInfo.Text = $"⚠ Минимум {d.MinDays} дн."; labelDiscountInfo.ForeColor = Color.FromArgb(220, 80, 80); }
                UpdateTotalPrice();
            }
        }
        ...
        private void UpdateServicesGrid() { dataGridViewServices.Rows.Clear(); foreach (var s in selectedServices) dataGridViewServices.Rows.Add(s.Name, s.Quantity, s.Price, s.Price * s.Quantity); }
        ...
        private void buttonLoadBooking_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string query = @"SELECT b.client_id, b.house_id, b.check_in_date, b.check_out_date, CASE hc.class WHEN 'Эконом' THEN 2000 WHEN 'Комфорт' THEN 3500 WHEN 'Люкс' THEN 6000 WHEN 'Премиум' THEN 9000 WHEN 'Бизнес' THEN 7000 ELSE 3000 END AS price FROM booking b JOIN house h ON b.house_id = h.id JOIN home_class hc ON h.home_class_id = hc.id WHERE b.id = @id";
                using (var cmd = new MySqlCommand(query, conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            int cid = reader.GetInt32("client_id"), hid = reader.GetInt32("house_id");
                            ...
                            CalculateStayDays(); UpdateTotalPrice();
                        }
                    }
                }
            }
        ...
        private void buttonAddService_Click(object sender, EventArgs e)
        {
            if (listBoxServices.SelectedItem is ServiceItem sel)
            {
                ...
                else selectedServices.Add(new ServiceItem { Id = sel.Id, Name = sel.Name, Price = sel.Price, Quantity = 1 });
                UpdateServicesGrid(); UpdateTotalPrice();
            }
        }
        ...
        {
            if (selectedServices.Count > 0 && MessageBox.Show("Удалить все услуги?", "", MessageBoxButtons.YesNo) == DialogResult.Yes)
            { selectedServices.Clear(); UpdateServicesGrid(); UpdateTotalPrice(); }
        }
        ...
        private void dateTimePickerCheckIn_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (dateTimePickerCheckOut.Value.Date < dateTimePickerCheckOut.MinDate) dateTimePickerCheckOut.Value = dateTimePickerCheckOut.MinDate;
            if ((dateTimePickerCheckOut.Value.Date - newCheckIn).TotalDays > MAX_STAY_DAYS) dateTimePickerCheckOut.Value = newCheckIn.AddDays(MAX_STAY_DAYS);
            CalculateStayDays(); UpdateTotalPrice();
        }

        private void dateTimePickerCheckOut_ValueChanged(object sender, EventArgs e)
        {
            DateTime checkIn = dateTimePickerCheckIn.Value.Date, checkOut = dateTimePickerCheckOut.Value.Date;
            if ((checkOut - checkIn).TotalDays > MAX_STAY_DAYS) { dateTimePickerCheckOut.Value = checkIn.AddDays(MAX_STAY_DAYS); MessageBox.Show($"Максимум {MAX_STAY_DAYS} дней."); return; }
            CalculateStayDays(); UpdateTotalPrice();
        }
        ...
        private void CalculateStayDays()
        {
            ...
            else { stayDays = 1; labelStayDays.Text = "1 день"; dateTimePickerCheckOut.Value = checkIn.AddDays(1); }
        ...
        private void comboBoxHouses_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            { houseDailyPrice = house.PricePerDay; labelHouseInfo.Text = $"Вместимость: {house.Capacity} чел."; comboBoxDiscount_SelectedIndexChanged(null, null); }
        }

        private void buttonCreateOrder_Click(object sender, EventArgs e)
        {
            if (comboBoxClients.SelectedItem == null || comboBoxHouses.SelectedItem == null) { MessageBox.Show("Выберите клиента и дом."); return; }
            ...
            {
                conn.Open();
                string q = @"SELECT COUNT(*) FROM (SELECT house_id FROM booking WHERE house_id=@h AND status IN ('pending','confirmed') AND check_in_date<@d2 AND check_out_date>@d1 UNION ALL SELECT house_id FROM check_in WHERE house_id=@h AND check_in_date<@d2 AND check_out_date>@d1) AS t";
                using (var cmd = new MySqlCommand(q, conn))
                {
                    cmd.Parameters.AddWithValue("@h", hid); cmd.Parameters.AddWithValue("@d1", checkIn); cmd.Parameters.AddWithValue("@d2", checkOut);
                    if (Convert.ToInt32(cmd.ExecuteScalar()) > 0) { MessageBox.Show("Дом занят на эти даты!"); return; }
                }
            }

            if (MessageBox.Show($"Оформить заказ на {totalPrice:N2}₽?", "", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
            ...
            try
            {
                using (var conn = DatabaseConnection.GetConnection())
                {
                    ...
                    {
                        string insert = @"INSERT INTO check_in (client_id, house_id, user_id, check_in_date, check_out_date, residence_time, house_total_price) VALUES (@c, @h, @u, @d1, @d2, @days, @p); SELECT LAST_INSERT_ID();";
                        int orderId;
                        using (var cmd = new MySqlCommand(insert, conn, tr))
                        { cmd.Parameters.AddWithValue("@c", cid); cmd.Parameters.AddWithValue("@h", hid); cmd.Parameters.AddWithValue("@u", uid); cmd.Parameters.AddWithValue("@d1", checkIn); cmd.Parameters.AddWithValue("@d2", checkOut); cmd.Parameters.AddWithValue("@days", stayDays); cmd.Parameters.AddWithValue("@p", houseCost); orderId = Convert.ToInt32(cmd.ExecuteScalar()); }
                        foreach (var s in selectedServices)
                            using (var cmd = new MySqlCommand("INSERT INTO check_in_services (order_number, service_id, quantity, service_total_price) VALUES (@o, @s, @q, @t)", conn, tr))
                            { cmd.Parameters.AddWithValue("@o", orderId); cmd.Parameters.AddWithValue("@s", s.Id); cmd.Parameters.AddWithValue("@q", s.Quantity); cmd.Parameters.AddWithValue("@t", s.Price * s.Quantity); cmd.ExecuteNonQuery(); }
                        tr.Commit();
                        ...
                        new ReceiptForm(orderId, cid, hid, houseCost, selectedServices, uid, checkIn, checkOut, stayDays).Show();
                        ClearForm();
                    }
                }
            }
        ...
        private void ClearForm()
        {
            ...
            comboBoxDiscount.SelectedIndex = 0;
            selectedServices.Clear(); UpdateServicesGrid();
            dateTimePickerCheckIn.Value = DateTime.Today; dateTimePickerCheckOut.Value = DateTime.Today.AddDays(1);
            CalculateStayDays(); UpdateTotalPrice();
        ...
        private void buttonBackToMenu_Click(object sender, EventArgs e)
        {
            ...
            { if (MessageBox.Show("Вернуться?", "", MessageBoxButtons.YesNo) == DialogResult.Yes) Close(); }
            else Close();
        }

        private void listBoxServices_SelectedIndexChanged(object sender, EventArgs e)
        { if (listBoxServices.SelectedItem is ServiceItem s) { textBoxServiceDescription.Text = s.Description; labelServicePrice.Text = $"Цена: {s.Price:N2}₽"; } }
